...
\large User Guide and Phase Offset Determination}
\author{EQSANS Instrument Team (Changwoo Do, Gergely Nagy, William Heller, Carrie Gao) \\Spallation Neutron Source, ORNL}
\date{\today}
...
\begin{abstract}
This guide describes the Python-based tools for EQSANS chopper calibration and phase verification. 
The two primary tools are \texttt{find\_edges\_ui.py} for extracting phase offsets from experimental 
monitor spectra, and \texttt{eqsans\_chopper\_sim.py} for simulating multi-chopper neutron transmission 
and verifying phase configurations. This document provides step-by-step workflows, explains the 
underlying physics, and includes practical examples for both 60\,Hz and 30\,Hz operating modes.
\end{abstract}
...
The EQSANS instrument at SNS uses a system of six disk choppers (1a, 1b, 2a, 2b, 3a, 3b) to define 
the wavelength band transmitted to the sample and detector. Precise timing (phase) of each chopper 
is critical for:
\begin{itemize}
    \item Selecting the desired wavelength range
    \item Minimizing frame overlap from adjacent source pulses
    \item Enabling frame-skipping mode for extended wavelength coverage at 30\,Hz
\end{itemize}

The calibration workflow involves two complementary approaches:
\begin{enumerate}
    \item \textbf{Experimental measurement}: Using \texttt{find\_edges\_ui.py} to analyze monitor 
    spectra and extract the actual chopper phase offsets from measured TOF edges.
    \item \textbf{Simulation verification}: Using \texttt{eqsans\_chopper\_sim.py} to predict the 
    wavelength bands for given phase settings and verify consistency with the control system 
    (hexaSub).
\end{enumerate}
...
Neutrons travel from the moderator to each chopper with time-of-flight (TOF) determined by their 
wavelength $\lambda$ and the chopper distance $L$:
\begin{equation}
...
Table~\ref{tab:choppers} lists the EQSANS chopper parameters.

\begin{table}[h]
\centering
\caption{EQSANS Chopper Parameters}
\label{tab:choppers}
\begin{tabular}{lccc}
\toprule
...
\textbf{60\,Hz mode}: Choppers rotate at 60\,Hz (period = 16,667\,$\mu$s), synchronized with the 
SNS source frequency. Each chopper opening window accepts neutrons from a single source pulse.
Figure~\ref{fig:diagram_60hz} shows a distance--time diagram for 60\,Hz operation with 
a 2.5\,\AA{} starting wavelength.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/diagram_60hz.png}
    \caption{Distance--time diagram for 60\,Hz mode (2.5--6.1\,\AA{} band). Shaded rectangles 
    indicate chopper closed periods. Blue trajectories show neutrons that successfully pass 
    through all four choppers (1b, 2b, 3a, 3b). The choppers define a single wavelength band 
    per source pulse.}
    \label{fig:diagram_60hz}
\end{figure}

\textbf{30\,Hz mode (frame-skipping)}: Choppers rotate at 30\,Hz (period = 33,333\,$\mu$s), but the 
source still operates at 60\,Hz. This creates two wavelength bands per chopper cycle:
\begin{itemize}
...
Different choppers align to different pulses in frame-skipping mode, enabling extended wavelength 
coverage with a gap between bands. Figure~\ref{fig:diagram_30hz} illustrates the frame-skipping 
configuration.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/diagram_30hz.png}
    \caption{Distance--time diagram for 30\,Hz frame-skipping mode. Two distinct wavelength bands 
    are visible: Band~1 (2.5--6.1\,\AA{}) from the current pulse, and Band~2 (9.8--13.4\,\AA{}) 
    from the previous pulse. The choppers 1b and 3a align to the previous pulse while 2b and 3b 
    align to the current pulse, creating the characteristic dual-band pattern.}
    \label{fig:diagram_30hz}
\end{figure}

\subsection{Monochromatic Mode}

For applications requiring a narrow wavelength band centered on a specific wavelength, all six 
choppers can be used in monochromatic mode. In this mode, hexaSub calculates phases such that:
\begin{itemize}
...
Figure~\ref{fig:diagram_mono} shows a configuration for 10\,\AA{} with $\pm$10\% spread 
(9.5--10.5\,\AA{}). Table~\ref{tab:mono_phases} lists the hexaSub-calculated OGC phases.

\begin{table}[h]
\centering
\caption{hexaSub OGC Phases for Monochromatic Mode (10\,\AA{} $\pm$ 10\%, SDD=4\,m)}
\label{tab:mono_phases}
\begin{tabular}{lccc}
\toprule
Chopper & Aligned to & Wavelength (\AA{}) & OGC Phase ($\mu$s) \\
\midrule
1a & $\lambda_{\min}$ (opening) & 9.50 & 14873.50 \\
1b & $\lambda_{\max}$ (closing) & 10.50 & 10462.74 \\
2a & $\lambda_{\min}$ (opening) & 9.50 & 4273.76 \\
2b & $\lambda_{\max}$ (closing) & 10.50 & 14639.37 \\
3a & $\lambda_{\min}$ (opening) & 9.50 & 9525.20 \\
3b & $\lambda_{\max}$ (closing) & 10.50 & 1143.60 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/diagram_mono.png}
    \caption{Distance--time diagram for monochromatic mode: 10\,\AA{} $\pm$ 10\% (9.5--10.5\,\AA{}). 
    All six choppers are active with phases calculated by hexaSub. The ``a'' choppers define the 
    lower wavelength cutoff while ``b'' choppers define the upper cutoff, creating a precisely 
    bounded wavelength band.}
    \label{fig:diagram_mono}
\end{figure}

%=============================================================================
\section{Tool 1: Interactive Edge Finder (\texttt{find\_edges\_ui.py})}
%=============================================================================
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/find_edges_60hz_example.png}
    \caption{Interactive Edge Finder showing a 60\,Hz monitor spectrum with a fitted rising edge. 
    The fit result panel shows the edge position (7335.4 $\pm$ 2.4\,$\mu$s), fit range, and 
    transition width.}
...
Table~\ref{tab:shortcuts} lists the interactive controls for the edge finder.
...
\begin{enumerate}
    \item Identify the rising and/or falling edges in the spectrum (transitions where intensity 
    changes sharply)
...
\end{equation}
where $t_0$ is the edge position, $\sigma$ is the transition width, and $I_L$, $I_R$ are the 
left/right intensity levels.
...
\begin{itemize}
    ...
    \item \textbf{Phase offset}: The difference between the current offset setting and the 
    calculated OGC
\end{itemize}
...
\subsection{Example: 30\,Hz Frame-Skipping Mode}

Figure~\ref{fig:find_edges_30hz} shows edge detection in 30\,Hz mode, where the frame period 
extends to 33,333\,$\mu$s.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/find_edges_30hz_example.png}
    \caption{Edge detection in 30\,Hz frame-skipping mode. The extended TOF range (0--33,333\,$\mu$s) 
    captures the full chopper period. The fitted edge at 10,685\,$\mu$s corresponds to a 2.5\,\AA{} 
    reference measurement.}
    \label{fig:find_edges_30hz}
\end{figure}

%=============================================================================
\section{Tool 2: Multi-Chopper Simulator (\texttt{eqsans\_chopper\_sim.py})}
%=============================================================================
...
\begin{itemize}
    \item \textbf{Control Panel (left)}: Frequency selection, chopper enable/disable, OGC and 
    mechanical offset inputs, hexaSub checkbox
    \item \textbf{Distance--Time Diagram (top right)}: Shows chopper positions, open/closed windows, 
    and neutron trajectories
    \item \textbf{Detector TOF Spectrum (bottom right)}: Histogram of neutrons arriving at the 
    detector
\end{itemize}

\subsection{Control Panel Elements}

Table~\ref{tab:sim_controls} describes the simulator controls.
...
Frequency (60/30 Hz) & Chopper rotation frequency; affects period and frame-skipping behavior \\
...
The EQSANS chopper control system uses \texttt{hexaSub.c} to calculate OGC phases for a requested 
wavelength. These phases include hardware calibration offsets ($\sim$29,000--30,000\,$\mu$s) that 
account for encoder alignment, cable delays, and other instrument-specific factors.
...
\begin{enumerate}
    ...
    \item For 30\,Hz mode, additional frame-skipping alignment corrections are applied
...
For 30\,Hz frame-skipping mode, enable choppers \textbf{1b, 2b, 3a, 3b} (the ``b-set'' plus 3a/3b).
...
Enter the OGC values calculated by hexaSub for your wavelength setting. For example, for 
2.5\,\AA{} at 30\,Hz with SDD=4\,m:
...
\begin{itemize}
    ...
    \item \textbf{Horizontal dashed lines}: Chopper positions with labels showing name, distance, 
    and effective OGC
...
Neutrons that intersect a shaded (closed) region are blocked. Only trajectories passing through 
all open windows reach the detector.

\subsection{Relationship to hexaSub.c}

The \texttt{hexaSub.c} code in the EPICS IOC calculates chopper phases using the \texttt{calc6Phases()} 
function. This section provides detailed explanations of phase calculation for all three operating modes.
...
\begin{itemize}
    \item \textbf{TOF calculation}: Time-of-flight from source to chopper position using 
    $\text{TOF} = d / (3956 \times 10^3) \times \lambda$ where $d$ is in mm and $\lambda$ in \AA{}
    
    \item \textbf{Centering adjustment}: After aligning an edge to a wavelength, the phase is adjusted 
    by $\pm \theta_{\text{opening}} \times t_{\text{half-angle}}$ to center the OGC (Opening Gap Center) 
    on the opening window
    
    \item \textbf{Calibration offset}: Hardware-specific offset ($\sim$15,000\,$\mu$s at 60\,Hz, 
    $\sim$30,000\,$\mu$s at 30\,Hz) added to account for encoder alignment, cable delays, etc.
...
The 60\,Hz mode has \emph{different} alignment logic depending on whether $\lambda_1 > 13$\,\AA{}, as shown in Table~\ref{tab:60hz_alignment}:
...
This swap prevents neutron leakage at different wavelength ranges.

\textbf{Origin of the 13\,\AA{} threshold}:

The 13\,\AA{} threshold is a hardcoded, empirically-determined constant in \texttt{hexaSub.c}. It is \emph{not} calculated from physical parameters---it was determined through testing and simulation to identify the crossover point where one alignment strategy becomes preferable to the other.

In the \texttt{calc6Phases()} function (around line 373 of \texttt{bl6chopper\_src/hexaSub.c}), the logic is:

\begin{verbatim}
/* we align the chopper differently between wl[1]>13 and wl[1]<=13
 * to stop leaks.  */
if (wl[1] > 13) {
    /* chopper 1A/1B: opening edge aligned to wl[1] (band start)
     * chopper 2A/2B: closing edge aligned to wl[2] (band end) */
} else {
    /* chopper 1A/1B: closing edge aligned to wl[2] (band end)
     * chopper 2A/2B: opening edge aligned to wl[1] (band start) */
}
\end{verbatim}

The physical reason for the swap relates to how chopper opening times interact with neutron TOF across different wavelength ranges:
\begin{itemize}
    \item At \emph{short wavelengths} ($\leq$13\,\AA{}), neutrons arrive quickly. Aligning choppers 1a/1b to the \emph{closing} edge of $\lambda_2$ better defines the long-wavelength cutoff and prevents frame overlap.
    \item At \emph{long wavelengths} ($>$13\,\AA{}), neutrons arrive later in the frame. Aligning choppers 1a/1b to the \emph{opening} edge of $\lambda_1$ better isolates the band from adjacent pulses.
\end{itemize}

The value 13\,\AA{} is approximately where the chopper opening time equals one half of the 60\,Hz frame period when accounting for the 1a/1b chopper distance ($\sim$5.67\,m). This crossover determines which alignment strategy minimizes neutron leakage.

The choppers 3a and 3b do \emph{not} swap:
\begin{itemize}
...
\subsubsection{30\,Hz Frame-Skipping Mode}

In 30\,Hz mode, choppers rotate at half speed, allowing two source pulses to contribute per 
chopper period. This creates \emph{two} wavelength bands. Only choppers 1b, 2b, 3a, 3b are used 
(indices 1, 2, 3, 4).
...
hexaSub computes four wavelengths for frame-skipping mode:
\begin{itemize}
...
Different choppers align to \emph{different} source pulses, as detailed in Table~\ref{tab:30hz_alignment}:
...
The ``previous pulse'' alignment means the TOF calculation \emph{always} includes subtraction 
of $1/60$\,s (one source period) for choppers 1b and 3a:
...
This creates the frame-skipping behavior where Band 2 neutrons (longer wavelengths) from the 
\emph{previous} source pulse arrive at the detector in the same time window as Band 1 neutrons 
from the \emph{current} pulse.

\textbf{Simulator frame-skipping corrections}:

When using hexaSub phases in the simulator (checkbox enabled), frame-skipping alignment 
corrections are applied only for choppers 3a and 3b:
\begin{itemize}
    \item 3a: $+33333.33$\,$\mu$s (full 30\,Hz period)
    \item 3b: $+33333.33$\,$\mu$s (full 30\,Hz period)
\end{itemize}

These corrections compensate for the period-wrapping in hexaSub's \texttt{fmod()} operation.
Choppers 1b and 2b require no additional alignment correction because hexaSub always subtracts
$1/60$\,s for 1b, resulting in phase values that already account for pulse alignment.

\subsubsection{Monochromatic Mode}

Monochromatic mode uses all six choppers to produce a narrow wavelength band (e.g., 10\,\AA{} 
$\pm$10\%). This mode operates at 60\,Hz and uses the 60\,Hz phase offsets.
...
Table~\ref{tab:mono_alignment} summarizes the edge alignment for monochromatic mode.
...
The code comments note: ``no funny business is required for wavelengths above or below 13\,\AA{} 
like is shown in the regular 60\,Hz mode.''

\textbf{Example: 10\,\AA{} $\pm$10\% (9--11\,\AA{})}:

Using hexaSub to calculate phases for 10\,\AA{} center wavelength with 10\% spread:
...
Table~\ref{tab:mode_comparison} summarizes the key differences between the three hexaSub operating modes.
...
\begin{tabular}{lccc}
\toprule
Feature & 60\,Hz Standard & 30\,Hz Frame-Skipping & Monochromatic \\
\midrule
...
\subsubsection{Using hexaSub Phases in the Simulator}

The simulator reverses hexaSub's operations to recover physical timing for ray-tracing:

\begin{enumerate}
    \item \textbf{Subtract calibration offset}: Removes the hardware-specific phase offset
    \item \textbf{Add frame-skipping corrections} (30\,Hz only): Compensates for period wrapping
    \item \textbf{Ray-trace}: Simulates neutron paths through all enabled choppers
\end{enumerate}

When the \textbf{hexaSub phases} checkbox is enabled, these corrections are applied automatically, 
allowing direct use of OGC values from hexaSub.
...
\subsection{Example 2: Verifying 30\,Hz Frame-Skipping Configuration}
...
\begin{itemize}
    \item EQSANS Instrument: \url{https://neutrons.ornl.gov/eqsans}
