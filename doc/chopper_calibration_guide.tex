...
\textbf{30\,Hz mode (frame-skip)}: Choppers rotate at 30\,Hz (period = 33,333\,$\mu$s), but the 
...
Different choppers align to different pulses in frame-skip mode, enabling extended wavelength 
coverage with a gap between bands. Figure~\ref{fig:diagram_30hz} illustrates the frame-skip 
configuration.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/diagram_30hz.png}
    \caption{Distance--time diagram for 30\,Hz frame-skip mode. Two distinct wavelength bands 
...
\begin{figure}[H]
    ...
    \caption{Edge detection in 30\,Hz frame-skip mode. The extended TOF range (0--33,333\,$\mu$s) 
...
This creates the frame-skip behavior where Band 2 neutrons (longer wavelengths) from the 
...
When using hexaSub phases in the simulator (checkbox enabled), frame-skip alignment 
...
\begin{enumerate}
    ...
    \item \textbf{Add frame-skip corrections} (30\,Hz only): Compensates for period wrapping
