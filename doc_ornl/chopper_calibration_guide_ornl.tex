...
\textbf{30\,Hz mode (frame-skipping)}: Choppers rotate at 30\,Hz (period = 33,333\,$\mu$s), but the source still operates at 60\,Hz. This creates two wavelength bands per chopper cycle:
\begin{itemize}
...
Figure~\ref{fig:diagram_30hz} illustrates the frame-skipping configuration.

\begin{figure}[ht]
    \centering
    \includegraphics[width=5.5in]{figures/diagram_30hz.png}
    \longcaption{Distance--time diagram for 30\,Hz frame-skipping mode.}{Two distinct wavelength bands are visible: Band~1 (2.5--6.1\,\AA{}) from the current pulse, and Band~2 (9.8--13.4\,\AA{}) from the previous pulse.}
...
\begin{figure}[ht]
    ...
    \longcaption{Edge detection in 30\,Hz frame-skipping mode.}{The extended TOF range (0--33,333\,$\mu$s) captures the full chopper period.}
...
This creates the frame-skipping behavior where Band 2 neutrons (longer wavelengths) from the \emph{previous} source pulse arrive at the detector in the same time window as Band 1 neutrons from the \emph{current} pulse.

\textbf{Simulator frame-skipping corrections}:

When using hexaSub phases in the simulator (checkbox enabled), frame-skipping alignment corrections are applied only for choppers 3a and 3b:
\begin{itemize}
