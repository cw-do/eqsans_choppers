 ...
 * chopper_phases5_value = *prec->f
 * chopper_phases6_value = *prec->g
 *
 * Outputs:
...
static long calc6Wavelengths(aSubRecord *prec) {
   double CHOPPER_PHASE_OFFSET[] = {14954.46, 14805.4, 14726.06, 14565.6, 15072.89, 14834.04 };
   double CHOPPER_ANGLE[] = {129.605, 179.989, 230.010, 230.007, 129.605, 179.989};
   double CHOPPER_LOCATION[] = {5675.7668, 7775.7668, 9497.8, 9507.8, 5660.1247, 7760.1247};
   ...
   for (i=0; i<6; i++) {
      //chopper_actual_phase[i]=chopper_set_phase[i] - CHOPPER_PHASE_OFFSET[i + m*6];
      chopper_actual_phase[i]=chopper_set_phase[i] - 2.0*CHOPPER_PHASE_OFFSET[i];
   ...
   if (frame_wl_1>=frame_wl_2) { /* too many frames later. So figure it out */
      int n_frame[] = {0,0,0,0,0,0};
      double c_wl_1[] = {0,0,0,0,0,0};
      double c_wl_2[] = {0,0,0,0,0,0};
      int passed=0;
 ...
 * NOTE:  These two are new March 7 2026
 * mono_wavelength = *prec->f
 * wave_spread = *prec->g
 *
...
static long calc6Phases(aSubRecord *prec) {
   ...
   double detector_location,det_dist;
   double mono_wavelength;
   double wave_spread;
   double chopper_location[] = {0.0, 5675.7668, 7775.7668, 9497.8, 9507.8, 5660.1247, 7760.1247};
   double chopper_opening[] =  {0.0, 129.605, 179.989, 230.010, 230.007, 129.605, 179.989};
   ...
   /* this is the detector distance, which appears to be send into the function in meters */

   /*the 0.707107 is an approximation of the distance from the detector center to a corner pixel in meters*/

   det_dist = *(double *)prec->b;

   detector_location = *(double *)prec->d + sqrt(det_dist*det_dist + 0.707107*0.707107 );
   detector_location *= 1000.0;
   bandwidth_at_60Hz = 3.956e6/detector_location/60.0;
   
   /* these parameters are used for the monochromatic mode */
   /* added March 7 2026 */
   monochromatic_mode = *(int *)prec->e;
   mono_wavelength = *(double *)prec->f;
   wave_spread = *(double *)prec->g;

   /* this conditional was modified on March 7 2026 */
   if (!monochromatic_mode){
	  wl[1] = *(double *)prec->a;
      wl[2] = wl[1] + bandwidth_at_60Hz;
   } else {
	  /* the wavelength spread is expressed as a fraction (i.e. 10% is "0.10") */
	  /* the wavelength times the spread has to be smaller than the maximum bandwidth */
	  if ((wave_spread*mono_wavelength) > bandwidth_at_60Hz) {
	     printf("the wavelength spread is larger than the available bandwidth at this detector distance\n");
         return -1;   
	  }
	  
      wl[1] = mono_wavelength - 0.5*wave_spread*mono_wavelength;
      wl[2] = mono_wavelength + 0.5*wave_spread*mono_wavelength;
   }
   //printf("in wl2 %f\n", wl[2]);
   ...
   if (chopper_speed_in_Hz == 30 && !monochromatic_mode) {
      double phase_offset[] = { 0, 29908.92, 29610.8, 29452.12, 29131.2, 30145.78, 29668.04 };
       ...
       */
      double tof_ch1 = chopper_location[1]/3.956e6*wl[3];
      if (tof_ch1 > (1.0/60.0)) {
         phase[1] = tof_ch1-1.0/60.0;
      } else {
         phase[1] = tof_ch1-1.0/60.0; /*Bhargavi, can we add -1.0/60.0  on this line as well? 3/19 5pm */
      }
      ...
      double tof_ch5 = chopper_location[5]/3.956e6*wl[3];
      if (tof_ch5 > (1.0/60.0)) {
         phase[5] = tof_ch5-1.0/60.0;
      } else {
         phase[5] = tof_ch5-1.0/60.0; /*Bhargavi, can we add -1.0/60.0  on this line as well? 3/19 5pm */
      }
   ...
   } else if (chopper_speed_in_Hz == 60 && !monochromatic_mode) {
      double phase_offset[] = { 0, 14954.46, 14805.4, 14726.06, 14565.6, 15072.89, 14834.04 };
      
     
      /* we align the chopper differently between wl[1]>13 and wl[1]<=13
       * to stop leaks.  */
      if (wl[1] > 13) { 
	     /* the chopper 1A (aka 1) and 1B (aka 5) pair */
         ...
         /* the chopper 2A (aka 2) and 2B (aka 6) pair */
         phase[2] = chopper_location[2]/3.956e6 * wl[2];
      ...
      /* check if T1 (Chopper 1, aka 1A) is not opened enough */
      ...
      /* check if T5 (Chopper 5, aka 1B) is not opened enough */
	  ...
	  /* this is the new mode of operation and is separated out to minimize confusion */
	  /* there is no reason to use monochromatic mode at 30 Hz. */
      double phase_offset[] = { 0, 14954.46, 14805.4, 14726.06, 14565.6, 15072.89, 14834.04 };
	  
	  /* chopper 1A (1) defines the lower wavelength wl[1] */
	  /* chopper 1B (5) defines the upper wavelength wl[2] */
	  /* chopper 2A (2) defines the lower wavelength wl[1] */
	  /* chopper 2B (6) defines the upper wavelength wl[2] */
	  /* chopper 3A (3) defines the lower wavelength wl[1] */
	  /* chopper 3B (4) defines the upper wavelength wl[2] */
	  
	  /* no funny business is required for wavelengths above */
	  /* or below 13 A like is shown in the regular 60 Hz mode. */
	  /* I might be wrong about this, so we may modify code later. */
	  
	  /* chopper 1 (1A) opening edge aligned to wl[1] */
      phase[1] = chopper_location[1]/3.956e6 * wl[1];
      phase[1] += beam_crosssection_adjust;
      phase[1] += pulse_width * wl[1];
      phase[1] += chopper_opening[1] * half_angle_to_sec; /* move to center */
      phase[1] = 1.0e6 * phase[1] + phase_offset[1];
      phase[1] = fmod(phase[1], frame_width);
	  
	  /* chopper 5 (1B) closing edge aligned to wl[2] */
      phase[5] = chopper_location[5]/3.956e6 * wl[2];
      phase[5] -= beam_crosssection_adjust;
      phase[5] -= chopper_opening[5] * half_angle_to_sec;
      phase[5] = 1.0e6 * phase[5] + phase_offset[5];
      phase[5] = fmod(phase[5], frame_width);
	  
	  /* chopper 2 (2A) opening edge aligned to wl[1] */
      phase[2] = chopper_location[2]/3.956e6 * wl[1];
      phase[2] += beam_crosssection_adjust;
      phase[2] += pulse_width * wl[1];
      phase[2] += chopper_opening[2] * half_angle_to_sec; /* move to center */
      phase[2] = 1.0e6 * phase[2] + phase_offset[2];
      phase[2] = fmod(phase[2], frame_width);
	  
	  /* chopper 6 (2B) closing edge aligned to wl[2] */
      phase[6] = chopper_location[6]/3.956e6 * wl[2];
      phase[6] -= beam_crosssection_adjust;
      phase[6] -= chopper_opening[6] * half_angle_to_sec;
      phase[6] = 1.0e6 * phase[6] + phase_offset[6];
      phase[6] = fmod(phase[6], frame_width);
	  
	  /* chopper 3 (3A) opening edge aligned to wl[1] */
      phase[3] = chopper_location[3]/3.956e6 * wl[1];
      phase[3] += beam_crosssection_adjust;
      phase[3] += pulse_width * wl[1];
      phase[3] += chopper_opening[3] * half_angle_to_sec; /* move to center */
      phase[3] = 1.0e6 * phase[3] + phase_offset[3];
      phase[3] = fmod(phase[3], frame_width);
	  
	  /* chopper 4 (3B) closing edge aligned to wl[2] */
      phase[4] = chopper_location[4]/3.956e6 * wl[2];
      phase[4] -= beam_crosssection_adjust;
      phase[4] -= chopper_opening[4] * half_angle_to_sec;
      phase[4] = 1.0e6 * phase[4] + phase_offset[4];
      phase[4] = fmod(phase[4], frame_width);
   } else {
